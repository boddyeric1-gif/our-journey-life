...
export async function trackClientEvent(event: AnalyticsEvent, props: Record<string, unknown> = {}) {
  ...
  const { error } = await supabase.from("app_events").insert({
    ...
    event,
    props: { ...props, ...(sessionId ? { session_id: sessionId } : {}), ...(attribution ? { attribution } : {}) },
  });
...
export async function initializeAnalytics() {
  ...
  try {
    ...
    if (existing?.id && existing.lastSeen && now - existing.lastSeen < SESSION_WINDOW_MS) {
  ...
  const attribution = readAttribution();
  if (attribution) {
    await supabase.rpc("capture_marketing_attribution", {
      _source: attribution.source ?? "direct",
      _medium: attribution.medium ?? null,
      _campaign: attribution.campaign ?? null,
      _content: attribution.content ?? null,
      _term: attribution.term ?? null,
    });
  }
  ...
  if (shouldStartSession) {
    await supabase.from("app_events").insert({ user_id: user.id, couple_id: profile?.current_couple_id ?? null, event: "session_started", props });
  }
}
