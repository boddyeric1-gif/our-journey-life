...
CREATE POLICY "invites self insert" ON public.invites FOR INSERT TO authenticated
  WITH CHECK (created_by = auth.uid() AND public.is_couple_member(couple_id));
CREATE POLICY "invites select participant" ON public.invites FOR SELECT TO authenticated
  USING (auth.uid() = created_by OR auth.uid() = used_by);
CREATE POLICY "invites update by creator" ON public.invites FOR UPDATE TO authenticated
...
CREATE POLICY "responses self or revealed"
  ON public.daily_responses FOR SELECT TO authenticated
  USING (
    user_id = auth.uid()
    OR (is_couple_member(couple_id) AND public.daily_both_submitted(couple_id, prompt_date))
  );
...
CREATE INDEX xp_events_user_kind_idx ON public.xp_events (user_id, kind);
CREATE UNIQUE INDEX xp_events_user_kind_dedupe_uidx ON public.xp_events (user_id, kind, dedupe_key);
...
CREATE POLICY "streaks self select" ON public.user_streaks FOR SELECT TO authenticated
...
CREATE POLICY "couple streaks member select" ON public.couple_streaks FOR SELECT TO authenticated
  USING (public.is_couple_member(couple_id));
...
-- =========================================================================
-- SHARES-COUPLE HELPER + PROFILE PARTNER VISIBILITY
-- =========================================================================
...
FOR SELECT TO authenticated USING (auth.uid() = id OR public.shares_couple_with(id));
...
ALTER TABLE public.couples ADD COLUMN subscription_tier public.subscription_tier NOT NULL DEFAULT 'free';
ALTER TABLE public.couples ADD COLUMN anniversary_date date;
...
CREATE POLICY "users insert own events" ON public.app_events FOR INSERT TO authenticated
  WITH CHECK (user_id = auth.uid());
CREATE POLICY "admins read all events" ON public.app_events FOR SELECT TO authenticated
